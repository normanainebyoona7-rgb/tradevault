...
// Analyzes chart with cross-source price verification.
...
export async function POST(request: Request) {
  try {
    ...
    if (signal.direction === "neutral") {
      return NextResponse.json({
        ...
        signal: {
          ...
          takeProfit3Price: 0,
          riskPips: 0,
          rewardPips1: 0, rewardPips2: 0, rewardPips3: 0,
    ...
    const pipValuePerLot = pair.includes("XAU") ? 10
      : pair.includes("XAG") ? 50
      : pair.includes("BTC") ? 1
      : pair.includes("ETH") ? 1
      : quote === "JPY" ? 9.5
      : 10;
    ...
    const lots = signal.riskPips > 0 ? Number((riskDollars / (signal.riskPips * pipValuePerLot)).toFixed(3)) : 0;
    ...
    const swingStart = signal.zone?.type === "demand" ? signal.zone.bottom : signal.zone?.top ?? currentPrice;
    const swingEnd = signal.zone?.type === "demand" ? signal.zone.top : signal.zone?.bottom ?? currentPrice;
    const swingRange = Math.abs(swingEnd - swingStart);
    const fibDirection = isLong ? 1 : -1;

    const fibonacci = {
      swingStart, swingEnd, swingRange,
    ...
    const sessionWeight =
      signal.confluences.session === "LONDON-NY OVERLAP" ? "Highest liquidity"
      : signal.confluences.session === "LONDON" || signal.confluences.session === "NEW YORK" ? "High liquidity"
      : signal.confluences.session === "ASIAN" ? "Lower volatility"
      : "Reduced liquidity";

    const tradeManagement: string[] = [];
    if (isLong) {
      tradeManagement.push(`Move SL to BE once price closes above ${signal.entry?.toFixed(5)}`);
      tradeManagement.push("Trail SL under new bullish large range candles");
    } else {
      tradeManagement.push(`Move SL to BE once price closes below ${signal.entry?.toFixed(5)}`);
    ...
    if (signal.orderType === "market") {
      ...
      uiOrderDesc = isLong ? "Market Buy — enter now" : "Market Sell — enter now";
    } else if (signal.orderType === "limit") {
      uiOrderType = isLong ? "BUY_LIMIT" : "SELL_LIMIT";
      uiOrderDesc = isLong ? "Buy Limit — place pending below price" : "Sell Limit — place pending above price";
    } else if (signal.orderType === "stop") {
      uiOrderType = isLong ? "BUY_STOP" : "SELL_STOP";
      uiOrderDesc = isLong ? "Buy Stop — place pending above price" : "Sell Stop — place pending below price";
    } else {
      uiOrderType = "NONE";
