...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      // New fields from the signal engine
      signalLabel,
      zone,
      confidence,
      score,
    ...
    const signal = await Signal.create({
      ...
      // Optional extras (ignored if schema doesn't define them)
    ...
    await sendTelegramSignal({
      pair,
      direction,
      signalLabel: signalLabel || direction.toUpperCase(),
      orderType: orderType || signal.orderType || "MARKET",
      orderTypeDescription: orderTypeDescription || "",
      entry,
      stopLoss,
      takeProfit1,
